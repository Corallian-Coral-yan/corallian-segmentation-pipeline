...
def train(config):
    if config["training"]["DoTraining"]:
        ...
        train_loader = DataLoader(
            train_dataset, batch_size=config["training"]["BatchSize"], shuffle=True, num_workers=8
        )
        val_loader = DataLoader(
            val_dataset, batch_size=config["training"]["BatchSize"], shuffle=False, num_workers=8
        )
