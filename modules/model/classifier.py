...
class DUC(nn.Module):
    def __init__(self, in_channels, up_factor):
        ...
        self.duc = nn.Conv2d(in_channels, in_channels * up_factor**2, kernel_size=3, padding=1)
...
class CoralSegmentationModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.feature_extractor = nn.Sequential(
            *list(self.feature_extractor.children())[:-2]).to(self.device)
        ...
        if self.duc_enabled:
            self.duc = DUC(in_channels=self.aspp_out_channels,
                        up_factor=self.duc_up_factor).to(self.device)  # Move to device
